...
func (self *CodeGenerator) generateExpr(mean analyse.Expr) string {
	...
	case *analyse.Equal:
		...
		switch expr.Opera {
		case "==":
			return self.equal(expr.GetType(), l, r)
		case "!=":
			return fmt.Sprintf("!(%s)", self.equal(expr.GetType(), l, r))
		default:
			return fmt.Sprintf("(%s) %s (%s)", l, expr.Opera, r)
		}
	case *analyse.Unary:
	...
	case *analyse.Select:
		c, t, f := self.generateExpr(expr.Cond), self.generateExpr(expr.True), self.generateExpr(expr.False)
		name := fmt.Sprintf("__v%d", self.varCount)
		self.varCount++
		self.writef("%s %s;", self.generateType(expr.GetType()), name)
		self.writef("if (%s) %s = %s; else %s = %s;", c, name, t, name, f)
		return name
	case *analyse.Tuple:
		t := self.generateType(expr.GetType())
		elems := make([]string, len(expr.Elems))
...
// 比较
func (self *CodeGenerator) equal(t analyse.Type, l string, r string) string {
	switch t.(type) {
